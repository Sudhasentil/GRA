...
import 'package:google_maps_in_flutter/Trip.dart';
import 'package:http/http.dart' as http;
import 'package:mobile_number/mobile_number.dart';
import 'Constants.dart';

class LRData extends StatefulWidget {
  const LRData({Key? key}) : super(key: key);

  @override
  State<LRData> createState() => _LRDataState();
}

class _LRDataState extends State<LRData> {
  //String _mobileNumber = '';
  String _mobileNumber = '9500293181';
  List<SimCard> _simCard = <SimCard>[];
  List LoadLRdata = [];
  var client = http.Client(); //EmpPersonal
  //List<String> ddMobileNo = [];
  ...
  Future<List<dynamic>> _fetchLRData() async {
    try {
      String API_URL = '$API/LRGPS/Mobile';
      print(_mobileNumber);
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          ...
          //print("hi");
          //print(LoadLRdata[0]['LG_DRI_MOBNO']);
          //dropdownItemlist = data[response[0]];

          //      print(LoadLRdata.length);
          //    int len = LoadLRdata.length;
          //  for (int item = 0; item < len; item++) {
//            ddMobileNo.add(LoadLRdata[item]['LG_DRI_MOBNO']);
          //        }
          //      print(_mobileNumber);
          //    print(ddMobileNo);
        });
              ...
              onChanged: (value) {
                setState(
                  () {
                    ...
                    //ddMobileNo[0] = _dropdownValues;
                    //ShowFields(_dropdownValues);
                  },
                );
              },

              items: ddMobileNo.map((var items) {
...
//          Container(
          ...
          //  Container(
          //    child: Text(_mobileNumber.substring(_mobileNumber.length - 10))),
          Container(
            child: Expanded(
              child: ListView.builder(
                ...
                itemBuilder: (context, index) {
                  //      print(_mobileNumber);
                  //_fetchLRData();
//                  if (LoadLRdata[index]["LG_DRI_MOBNO"] ==
//                      _mobileNumber.substring(_mobileNumber.length - 10)) {
                  //    print(_mobileNumber);
                  return Column(
                    children: <Widget>[
                      ListTile(
